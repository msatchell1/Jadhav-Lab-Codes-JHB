...
%                 'position',{},'plotBox',false);
            ...
            % MES: OdorMeans field is deleted in Object_selectivity.m, and
            % I believe the information is simply moved to
            % SuperRat.units.OdorSelective. So I am applying that change
            % here.
              boxScatterplot(spkevs(keeptrials),double(odorid(keeptrials)==0),'yLabel','Rate',...
                    'xLabels',{sprintf('Odor Left %.2f Hz',SuperRat(ses).units(UseCells(i)).OdorSelective{1,5}),...
                    sprintf('Odor Right %.2f Hz',SuperRat(ses).units(UseCells(i)).OdorSelective{1,6})},...
                    'position',{},'plotBox',false);
            % now line bar
            ...
            set(gca,'XTick',[0 1],'XTickLabel',{sprintf('Odor Left %.2f Hz',SuperRat(ses).units(UseCells(i)).OdorSelective{1,5}),...
                sprintf('Odor Right %.2f Hz',SuperRat(ses).units(UseCells(i)).OdorSelective{1,6})});
            xlim([-0.5 1.5]); ylabel('Firing rate, Hz');
            title(sprintf('A=%.2f B=%.2f, p=%.3f', SuperRat(ses).units(UseCells(i)).OdorSelective{1,5},...
                SuperRat(ses).units(UseCells(i)).OdorSelective{1,6},...
        ...
        title(sprintf('Odor Delta Rate: %.2f p=%.4f \n Odor L %.2f, odor R %.2f  p=%.4f',...
            max(abs(SuperRat(ses).units(UseCells(i)).taskResponsive(:,2))),...
            min(SuperRat(ses).units(UseCells(i)).OdorResponsive(:,4)),...
            max([SuperRat(ses).units(UseCells(i)).OdorSelective(1,5:6) 0]),...
            max([SuperRat(ses).units(UseCells(i)).OdorSelective(2,5:6) 0]),...
            SuperRat(ses).units(UseCells(i)).OdorSelective(1,2)));
        % The following two lines I replaced with OdorSelective above:
